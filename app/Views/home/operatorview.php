    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#myTable').DataTable({
                ...
                // lengthChange: false,
                ajax: {
                    url: '<?= base_url('public/operator/data'); ?>',
                    type: 'POST'
                },
                columns: [{
                        data: 'idOperator'
                    ...
                    },
                    {
                        render: function(data, type, row) {
                            return `${buttonDelete(row.idOperator)} ${buttonUpdate(row.idOperator, row.namaOperator)}`;
                        }
                    },
                ]
            });
        ...
        function buttonUpdate(idOperator, namaOperator) {
            return `<a href="#" class="btn icon btn-primary" data-bs-toggle="modal" data-bs-target="#modalUpdate" data-id="${idOperator}" onclick="getData('${idOperator}','${namaOperator}')"> <i class="bi bi-pencil"></i></a>`;
        }

        let idOperatorUpdate = null;

        function getData(idOperator, namaOperator) {
            idOperatorUpdate = idOperator;
            $('#namaOperatorUpdate').val(namaOperator);
        }

        function update() {
            var data_input = $('#formUpdate').serialize();
            // if (!checkValidate($('#idOperator').val(), $('#namaOperator').val())) {
            //     return;
            // }
            $.ajax({
                    method: "POST",
                    url: "<?= base_url('public/operator/update'); ?>" + '/' + idOperatorUpdate,
                    data: data_input,
                    dataType: "json"
                })
                .done(function(res) {
        ...
        function js_hapus(id) {
            ...
            $.ajax({
                    method: "POST",
                    url: "<?= base_url('public/operator/hapus'); ?>",
                    data: {
                        id: id_delete
                    },
                    dataType: "json"
        ...
        function simpan() {
            ...
            $.ajax({
                    method: "POST",
                    url: "<?= base_url('public/operator/simpan'); ?>",
                    data: data_input,
                    dataType: "json"
                })
                .done(function(res) {
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-simpan">
                        ...
                        <input type="text" class="form-control" id="idOperator" name="idOperator">
    ...
    <div class="modal fade text-left" id="modalUpdate" tabindex="-1" role="dialog" aria-labelledby="myModalLabel33" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="formUpdate">
                        <input type="hidden" class="form-control" id="idOperatorUpdate" name="idOperator">
                        <label>Nama Operator</label>
                        <input type="text" class="form-control" id="namaOperatorUpdate" name="namaOperator">
                    </form>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-light-secondary" data-bs-dismiss="modal">
