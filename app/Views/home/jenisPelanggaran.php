    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#myTable').DataTable({
                ...
                // lengthChange: false,
                ajax: {
                    url: '<?= base_url('public/Pelanggaran/datajenispelanggaran'); ?>',
                    type: 'POST'
                },
                columns: [{
                        data: 'idPelanggaran'
                    ...
                    },
                    {
                        ...
                        render: function(data) {
                            return `${buttonDelete(data)} ${space()} ${buttonUpdate(data)}`;
                        }
                    },
                ]
            });
        ...
        function js_hapus(id) {
            ...
            $.ajax({
                    method: "POST",
                    url: "<?= base_url('public/Pelanggaran/hapusjenispelanggaran'); ?>",
                    data: {
                        id: id_delete
                    },
                    dataType: "json"
        ...
        function simpan() {
            ...
            $.ajax({
                    method: "POST",
                    url: "<?= base_url('public/Pelanggaran/simpan'); ?>",
                    data: data_input,
                    dataType: "json"
                })
                .done(function(res) {
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable " role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="form-simpan">
                        <label>ID</label>
                        <input type="text" class="form-control" id="idPelanggaran" name="idPelanggaran">
                        <label>Pelaku Pelanggar</label>
                        <!-- <input type="text" class="form-control" id="idPelaku" name="idPelaku"> -->
                        <select id="idPelaku" name="idPelaku" class="form-control">
                            <?php foreach ($pelakuM as $key => $value) : ?>
                                <option value="<?= $value->idPelaku ?>"><?= $value->jenisPelaku ?> </option>
                            <?php endforeach; ?>
                        </select>
